...
// targets and nothing else. The *.skip and *.bak spec copies stay in srcs;
// no tool reads them, but they cost nothing as key material.
...
const coverageGate = S.Coverage.Gate({
	report: testCoverage,
	thresholds: { lines: 50, functions: 52, branches: 41, statements: 46 },
})
...
// Semver as a gate: baseline is the last published @tevm/receipt-manager
...
// `smthrs //packages/receipt-manager:check`.
const check = S.Suite({
