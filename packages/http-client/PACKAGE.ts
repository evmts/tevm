...
const Shell = scopedShell('packages/http-client')

// Follows the packages/evm/PACKAGE.ts exemplar with one removal: there is no
// vitest.config.ts, so vitest runs on defaults and declares no coverage
// thresholds. The exemplar's coverage gate is absent; the coverage run stays.
