...
const Shell = scopedShell('packages/effect')

// Follows the packages/evm/PACKAGE.ts exemplar with one removal: this package
// has no typecheck script (build:types's tsc pass is the typecheck), so the
// exemplar's typecheck target is absent.
...
// is vitest in watch mode with coverage; `test:ui` is the vitest UI.
