...
// package's own checks so a fix cannot regress a neighboring hardfork, and
// the no-mocks lint keeps the repro honest.
const conformanceTriage = S.Agent.Diff({
	...
	data: [S.gitDiff(), test.traceCompare, test.gstIsolate, test.execSpecIsolate],
	changes: ['packages/evm/src/**', 'packages/vm/src/**', 'packages/state/src/**', 'test/**'],
	gates: [test.conformanceFast, evm.typecheck, evm.test, root.noMocksLint],
