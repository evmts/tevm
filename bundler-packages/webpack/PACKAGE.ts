...
const Shell = scopedShell('bundler-packages/webpack')
...
// tsconfig outDir (types/). There is no vitest.config.ts and no
// test:coverage script, so there are no testCoverage or coverageGate
// targets. Skipped scripts: `build` is the nx run-many wrapper, `test` and
...
// typecheck. tsconfig has no spec exclude, so the specs are typechecked too
// and tests are key material.
...
// Semver gate against the last published @tevm/webpack-plugin declarations.
