...
const Shell = scopedShell('bundler-packages/rspack')
...
// coverageGate: no thresholds exist to enforce. Skipped scripts: `build` is
// the nx run-many wrapper, `test` and `test:ui` are vitest watch and UI
// sessions, and `format`/`format:check` run only biome's formatter, which
// the lint and format targets cover through biome check.
...
// Semver gate against the last published @tevm/rspack-plugin declarations.
