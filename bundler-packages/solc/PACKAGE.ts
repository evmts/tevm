...
const Shell = scopedShell('bundler-packages/solc')
...
// tsconfig outDir (types/), plus the solc:version helper. vitest.config.ts
// declares no thresholds, so there is no coverageGate. Skipped scripts:
// `all` is an aggregate the check suite replaces, `build` is the nx run-many
// wrapper, `test` and `test:ui` are vitest watch and UI sessions, and
// `format`/`format:check` run only biome's formatter, which the lint and
// format targets cover through biome check.
...
// The solc:version helper script. Not part of the library sources.
const versionScript = S.file('scripts/version/index.cjs')
...
// solc:version. Installs the requested solc release from npm in a temp
// directory to print its version hash, so it needs the network. The version
// argument is passed at invocation time.
const solcVersion = Shell.Run({
	bin: S.Runtime.bin,
	args: ['scripts/version/index.cjs'],
	data: [versionScript],
...
// Semver gate against the last published @tevm/solc declarations.
...
const check = S.Suite({
	tests: [lint, typecheck, test, testCoverage, depsLint, packageLint],
})
