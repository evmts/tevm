...
// packages/evm/PACKAGE.ts is the exemplar for this shape. Like the
// exemplar, this package emits declarations with tsup only, so there is no
// `declarations` target. Skipped scripts: `build` is the nx run-many
...
const coverageGate = S.Coverage.Gate({
	report: testCoverage,
	thresholds: { lines: 97, functions: 80, branches: 100, statements: 94 },
})
...
// Semver gate against the last published @tevm/unplugin declarations.
