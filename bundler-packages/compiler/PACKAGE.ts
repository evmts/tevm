...
const Shell = scopedShell('bundler-packages/compiler')
...
// vitest.config.ts includes root-level `*.{test,spec}.*` alongside src, so
// source.spec.ts at the package root is part of the suite. It imports
// ./source and mocks ./TestContract.js, so both root files are test data.
const tests = S.Filegroup({
	srcs: S.glob(['src/**/*.spec.ts', 'src/**/*.test.ts', 'src/**/__snapshots__/**', '*.spec.ts']),
})

const sourceFixture = S.file('source.ts')
const testContractFixture = S.file('TestContract.d.ts')
...
// test:run. The 0.8.20 shadow-compilation cases load the pinned compiler
// build from the Solidity binary CDN, so egress is an explicit capability.
const test = Shell.Test({
	bin: S.NodeModule.Bin('vitest'),
	args: ['run'],
	data: [srcs, tests, sourceFixture, testContractFixture, deps, vitestConfig, tsconfig],
...
// Semver gate against the last published @tevm/compiler declarations.
